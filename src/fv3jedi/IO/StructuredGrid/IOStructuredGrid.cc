...
// -------------------------------------------------------------------------------------------------
// Abort if a NetCDF call returns a non-zero status code.
...
// -------------------------------------------------------------------------------------------------
// Read a global integer attribute named `attrName` from an open NetCDF file.
// Falls back to the length of dimension `dimName` when the attribute is absent.
static int readGlobalIntAttrOrDimLen(int ncid,
                                     const char * attrName,
                                     const char * dimName) {
  // First, try to read the global integer attribute.
  int val;
  if (nc_get_att_int(ncid, NC_GLOBAL, attrName, &val) == NC_NOERR) {
    return val;
  }

  // Attribute not present: fall back to the length of the named dimension.
  int dimId;
  nc_rc(nc_inq_dimid(ncid, dimName, &dimId),
        std::string("nc_inq_dimid for fallback dim '") + dimName + "'");
  size_t len;
  nc_rc(nc_inq_dimlen(ncid, dimId, &len),
        std::string("nc_inq_dimlen for fallback dim '") + dimName + "'");
  return static_cast<int>(len);
}

// -------------------------------------------------------------------------------------------------
// Infer the Atlas grid string (e.g. "F96") for a regular Gaussian grid whose
// output file has `im` longitude points and `jm` latitude rows.
//
// Strategy: for a Regular Gaussian Grid F<N>, Atlas defines ny = 2*N and
// nx = 4*N for all rows.  Given (im, jm) we iterate over candidate N values
// near jm/2, construct the Atlas grid, and verify nx == im && ny == jm.
// Aborts with a diagnostic message if no matching grid is found.
static std::string inferAtlasGaussianGridString(int im, int jm) {
  // Build a small list of candidate F<N> grid strings.
  // For a Regular Gaussian Grid F<N>: ny = 2*N and nx = 4*N, so N = jm/2.
  // We also try N±1 to be robust against non-standard configurations.
  const int nBase = jm / 2;
  std::vector<std::string> candidates;
  for (int delta : {0, -1, 1}) {
    const int n = nBase + delta;
    if (n > 0) {
      candidates.push_back("F" + std::to_string(n));
    }
  }

  // Test each candidate against the target (im, jm).
  for (const std::string & gridStr : candidates) {
    try {
      const atlas::StructuredGrid grid(gridStr);
      if (!grid) continue;
      // For a regular Gaussian grid all rows have the same number of points,
      // so nxmax() equals nx(j) for every j.
      if (grid.ny() == jm && grid.nxmax() == im) {
        oops::Log::info() << "IOStructuredGrid: inferred Atlas Gaussian grid string '"
                          << gridStr << "' for im=" << im << " jm=" << jm << std::endl;
        return gridStr;
      }
    } catch (...) {
      // Not a valid grid string; try the next candidate.
    }
  }

  // No candidate matched — abort with a clear diagnostic.
  std::ostringstream oss;
  oss << "IOStructuredGrid::inferAtlasGaussianGridString: "
      << "could not find an Atlas Regular Gaussian grid string matching "
      << "im=" << im << " jm=" << jm << ". "
      << "Candidates tried:";
  for (const auto & s : candidates) oss << " " << s;
  ABORT(oss.str());
  return "";  // unreachable
}
